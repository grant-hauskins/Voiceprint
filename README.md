...
Double-click `Voiceprint.cmd` in the repository root, or run `scripts\dev.ps1 up`. It provisions the local credentials, asks once for the controller name, address and email (saved to ignored `data\launcher.env`; they appear verbatim in every written release), builds the jar if missing, then starts the worker, the API, the cloudflared MCP tunnel and the agent runtime in that one window, verifies the public MCP URL, and opens `http://127.0.0.1:8080/ui`. The page connects to the runtime by itself; nothing is pasted. Setup, releases and enrollment appear as an inescapable step dialog over the console; the console itself (agents with their speaking orbs, microphone state, transcript, room status, server-side MCP evidence) is what you see once the room is enrolled. The rest of a session happens there:

1. **Conversation runtime** panel: paste the OpenAI API key (held only in the runtime process, never written to disk) and enter each person's full name and email/phone; two to four people. Under **Agents**, one card per agent: name, voice, eagerness, standing instructions typed in or loaded from a text file ("always answer in haiku", "only words that start with A", a summary of what your agent should know), and optionally the roster name it **speaks for**. *Add agent* gives another person their own agent; each card is a separate provider session with only its own instructions, up to four. What the panel shows is what is saved on this computer under `data\agents` (one file per agent name) and prefilled next time; the room rules about who spoke and when to speak still apply on top. *Create room* posts the roster; the microphone stays closed.
...
4. Press **Start conversation**; the microphone indicator turns live. Each agent card shows a gradient orb that glows and pulses while that agent is speaking, with Speak/Hold/Cancel and eagerness beside it; the transcript, floor, provider-declared tool activity and the server's own MCP proof sit in the same grid. *Releases* in the header reopens the release list at any time. *End conversation* closes the microphone and ends the room; *Withdraw* on any release stops everything and starts destruction.
...
## Agents that represent someone (v3)

Scope and decisions: [BUILD_SPEC_V3.md](docs/BUILD_SPEC_V3.md); contract: the V3 section of [API.md](docs/API.md); build plan and the decisions taken without Grant: [V3_STREAMS.md](docs/V3_STREAMS.md). **Built and unit-tested; not yet exercised in a live room** (that needs two people, a provider key and the tunnel; see `HANDOFF_PROMPT.md` §J).

- A room can be a `negotiation`: two voice **advocates**, each speaking for one person, plus one text-only **arbitrator** that never has a voice, never claims the floor and never opens a realtime session. Every agent gets a shared instruction layer; each advocate additionally gets its own principal's structured, versioned objective (shareable position plus private constraint values), injected into its own prompt by the runtime and never exposed through any tool.
- The arbitrator holds both objectives, reads the transcript through the same MCP tools the advocates use (so its calls show in the proof panel), generates only when a cheap tiered check says there is something to add, and writes to a **notes board** (default visible) or a **raw agent channel** (hidden on the console until every person in the room reveals it; the API enforces that, the page only reflects it). Its two override tags, `OBJECTIVE_ACHIEVED` and `REFOCUS_NEEDED`, let it prompt an advocate past the normal waiting rules, but only after a separate verifier call confirms the claim from the evidence.
- A server-side **redaction guard** runs over every channel, board and summary write against every registered constraint value (digits, thousands separators, currency, `300k`, spelled-out numbers, dated phrases) and stores `[withheld]` instead; the same guard mirrors in the runtime and cuts an advocate's speech when the streamed transcript matches. What it deliberately misses is listed in the contract. **Residual risk: audio already played before the matching transcript arrives cannot be recalled**; the instruction layer is the first line, the guard the backstop.
- Ending the conversation writes a closing summary (agreements, open items, next steps) as a retained database record with its own 30-day deadline **before** the room is ended and destroyed, and also to `data/summaries/<session>.md` (`--summary-file PATH` to move it, `--no-summary-file` to skip it).
- All of it requires a third disclosure scope, `negotiation_text`, in every person's written release (the notice text changed, so older rooms must be re-signed). Attribution labels stay `similarity_based_uncalibrated`; this is a demo, not a tool for consequential negotiations ([CALIBRATION.md](docs/CALIBRATION.md)). The advocate-voice residual risk above is accepted for v3 (Grant, 2026-09-08).
- Transcript accuracy (v3.1): any utterance can be reviewed in the console (fix the words, pick the right speaker); a speaker fix feeds the existing segment correction so the profiles improve during the room. The gate matches a spoken name to an agent tolerantly (`Brian` in the transcript still addresses `Ryan`; `been` never addresses `Ben`). A person who signs the optional `voice_profile_retention` release keeps their voiceprint (embedding only, never audio) after the room so later rooms start from it and refine it, until they delete it in the console, withdraw from a room, or three years pass without a session. The notice names the AI providers the operator has configured (`VOICEPRINT_PROVIDERS`, default OpenAI) and states that other providers may be configured under the same release.
