...
- Ending the conversation writes a closing summary (agreements, open items, next steps) as a retained database record with its own 30-day deadline **before** the room is ended and destroyed; no file is written unless the operator passes `--summary-file`.
- All of it requires a third disclosure scope, `negotiation_text`, in every person's written release (the notice text changed, so older rooms must be re-signed). Attribution labels stay `similarity_based_uncalibrated`; this is a demo, not a tool for consequential negotiations ([CALIBRATION.md](docs/CALIBRATION.md)).
