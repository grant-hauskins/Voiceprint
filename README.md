...
1. **Conversation runtime** panel: paste the OpenAI API key (held only in the runtime process, never written to disk) and enter each person's full name and email/phone; two to four people. *Create room* posts the roster; the microphone stays closed.
...
Agents are configured in `scripts/agents.toml`: Ava (voice `marin`, balanced) and Ben (voice `cedar`, quiet), both on OpenAI Realtime `gpt-realtime-2.1`, with a preferred output device per agent. xAI and Gemini adapters are stubs.
