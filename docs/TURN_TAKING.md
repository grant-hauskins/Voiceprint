...
**Spoken-leak guard.** Advocates keep their realtime voice, so their words reach the room before the runtime sees them. The runtime mirrors the server's redaction guard over the streamed `response.output_audio_transcript.delta` text and, on a match with the advocate's own principal's constraint values, cancels the response and flushes playback (the existing cancel path, which also keeps the microphone muted through the hardware tail); the stored utterance text is redacted. This is best effort: **audio already rendered before the matching transcript delta arrives cannot be recalled.** The shared instruction layer that lists the values never to say is the first line of defense; this guard is the backstop; `docs/V3_STREAMS.md` records this as the largest open risk in v3, accepted by Grant on 2026-09-08.

**Tolerant addressing (v3.1).** Local ASR substitutes similar-sounding names ("Brian" for "Ryan"), and a gate that only matched the exact name left the addressed agent silent. `addressed()` now also matches any word of the line to an agent name when both are at least four characters and, after a light phonetic normalization (casefold, `y`→`i`, `ph`→`f`, `ck`→`k`, doubled letters collapsed), their similarity ratio is at least 0.85 ("brian"/"ryan" scores 0.889; "great"/"grant" scores 0.8 and is rejected); names of three letters or fewer match exactly only, so "been" never wakes Ben. A review made in the console after a line was already polled does not rewrite the gate's history of that line; only rows that arrive reviewed are seen reviewed. The same rule governs "a line naming another agent is not an opening for me". When the match was inexact the pre-reply note tells the agent how the transcript spelled its name, and the shared instruction layer tells every agent that the transcript may misspell or substitute names. Reviewed lines (`label: reviewed`, a human correction of words or speaker made in the console) count as reliable attribution, never as a reason to ask who spoke.
