...
Policy configuration: `VOICEPRINT_CONTROLLER_NAME`, `VOICEPRINT_CONTROLLER_ADDRESS` and `VOICEPRINT_CONTROLLER_EMAIL` are all required and must be nonempty to collect a release. Grant supplies the individual or entity and counsel reviews that decision; no name is invented by the application. Policy and method versions are fixed in code and included in the notice hash. Public `GET /privacy/notice` returns `{configured, controller_name, controller_address, controller_email, policy_version, consent_method_version, purpose_id, notice_text, notice_sha256, retention_text, vendors}`. It needs no API token and contains no participant personal data. The retention policy is also a static public document suitable for the operator's public website; the loopback GUI alone does not make the policy public to everyone.

`POST /privacy/rooms` with `{session_id, participants:[{id,name,contact}], purpose_id:"live_conversation_v1"}` creates only a pending privacy room/roster (no audio, no embeddings), returns 201 `{session_id, state:"pending"}`. Name is the participant's full typed name; contact is their typed email or phone, stored but unverified. Require configured `VOICEPRINT_API_TOKEN` and valid operator authentication (refuse the consent flow if absent), exact local Origin, validated IDs, distinct participants. Room membership is fixed for a capture epoch; changes require capture stop and fresh authorization. Existing v2 `/participants` remains agent-only.
...
The returned scopes are EFFECTIVE permissions: `openai_audio` additionally requires OpenAI review; `hosted_mcp` additionally requires OpenAI and Cloudflare review. `allowed` and `local_processing` do not imply either disclosure permission. The runtime requires both hosted scopes before connecting its hosted voice adapter. Authenticated local `GET /privacy/rooms` returns `{rooms:[{session_id,state}]}` including pending rooms, ordered newest first, without contacts/signatures. This allows the GUI to find a runtime-created consent room. The existing `/speaker/sessions` response remains compatible; hosted `list_sessions` must never reveal pending or unauthorized rooms.
...
The initial UI uses `local-kiosk-checkbox-name-v1`: the operator enters full name and email/phone, then the participant personally reviews the notice and checks/submits the explicit electronic-release checkbox. Record identity method `operator_assisted_unverified_contact`, the typed name/contact, server timestamp, exact consent text/version/hash and authenticated operator identity derived from the API token credential identifier (never store the raw token). This is NOT verified participant identity; disclose the limitation accurately. Grant explicitly chose this spike flow, with individual accounts/verified contact deferred to a later stream. The bearer token authenticates the operator/client, never supplies a human's consent. A subject or verified legally authorized representative must sign; representative flows are not implemented in this build and collection must be refused when they are needed.

Commit the written release BEFORE opening the microphone. Then prompt the participant's eight-second enrollment statement to begin: `I, [full name], consent to Voiceprint collecting my voiceprint for [rendered purpose] today.` Hash the exact decoded PCM bytes actually submitted for enrollment with SHA-256 on the server and attach the hash to that pre-existing participant consent in the successful enrollment transaction. The spoken statement corroborates the earlier release; no audio capture, hashing of newly captured audio, transcription or biometric enrollment is permitted to obtain the prior written release itself. Never infer consent from recognizing a spoken phrase. Rejected enrollment does not fabricate a successful audio link; preserve the actual written release and require an authorized retry. Do not retain the raw statement solely as signature evidence. Clear the audio hash when protected session data is destroyed; retain minimal non-biometric release evidence under its separately approved legal-evidence schedule.
...
Authenticated local `GET /speaker/session/{id}/destruction` returns `{session_id,state,verified,jobs:[{job_id,state}],items:[{destination,state}],failures:[]}`. `verified` is true only if all registered destruction items have verified completion; failures are sanitized messages without payloads/credentials. After hosted egress, a vendor-verification item remains pending until a trustworthy receipt or reviewed account-specific no-retention mechanism establishes that no vendor copy remains. Closing a socket is not such evidence. The GUI clears protected room content on revocation/destruction rather than displaying cached transcript rows. This endpoint must not be exposed as an MCP tool.
...
- This runtime milestone keeps bounded operational event metadata in memory only; even an explicit disk-events option is refused until artifact tracking and protected storage are implemented. Score before clearing memory at purpose completion or withdrawal. Eviction and missing playback evidence must be reported as incomplete. This restriction does not remove the API's separately protected session-scoped MCP proof or authorize retaining legacy logs.
